...
router.get("/class-types", requireAuth, async (req, res, next) => {
  try {
    const rows = await db.select().from(classTypesTable).where(eq(classTypesTable.isActive, true)).orderBy(classTypesTable.name);
    res.json(rows);
  } catch (err) {
    next(err);
  }
...
router.post("/class-types", requireAuth, async (req, res, next) => {
  try {
    ...
    const [classType] = await db.insert(classTypesTable).values(data).returning();
...
router.patch("/class-types/:id", requireAuth, async (req, res, next) => {
  try {
    ...
    const data = schema.parse(req.body);
    const [updated] = await db.update(classTypesTable).set({ ...data, updatedAt: new Date() }).where(eq(classTypesTable.id, req.params.id)).returning();
...
router.get("/class-sessions", requireAuth, async (req, res, next) => {
  try {
    ...
    if (status) conditions.push(eq(classSessionsTable.status, status as any));
...
router.post("/class-sessions", requireAuth, async (req, res, next) => {
  try {
    ...
    const [classType] = await db.select().from(classTypesTable).where(eq(classTypesTable.id, data.classTypeId));
...
router.patch("/class-sessions/:id", requireAuth, async (req, res, next) => {
  try {
    ...
    const [updated] = await db.update(classSessionsTable).set(updateData).where(eq(classSessionsTable.id, req.params.id)).returning();
...
router.post("/bookings", requireAuth, async (req, res, next) => {
  try {
    const schema = z.object({ memberId: z.string().uuid(), sessionId: z.string().uuid() });
    ...
    const existing = await db.select({ id: bookingsTable.id }).from(bookingsTable)
      .where(and(eq(bookingsTable.memberId, memberId), eq(bookingsTable.sessionId, sessionId), eq(bookingsTable.status, "confirmed")));
    if (existing.length > 0) throw new AppError(400, "Already booked for this session");

    if (session.currentBookings >= capacity) {
      const [lastWaitlist] = await db.select({ position: waitlistEntriesTable.position }).from(waitlistEntriesTable)
        .where(eq(waitlistEntriesTable.sessionId, sessionId)).orderBy(desc(waitlistEntriesTable.position)).limit(1);
      const position = (lastWaitlist?.position ?? 0) + 1;
      const [entry] = await db.insert(waitlistEntriesTable).values({ memberId, sessionId, position, expiresAt: new Date(Date.now() + 10 * 60 * 1000) }).returning();
      res.status(201).json({ waitlisted: true, position, entry });
      return;
    }

    const [booking] = await db.insert(bookingsTable).values({ memberId, sessionId }).returning();
    await db.update(classSessionsTable).set({ currentBookings: session.currentBookings + 1, updatedAt: new Date() }).where(eq(classSessionsTable.id, sessionId));
...
router.delete("/bookings/:id", requireAuth, async (req, res, next) => {
  try {
    ...
    const [booking] = await db.select().from(bookingsTable).where(eq(bookingsTable.id, req.params.id));
    if (!booking) throw new AppError(404, "Booking not found");

    await db.update(bookingsTable).set({ status: "cancelled", cancelledAt: new Date(), cancellationReason: reason }).where(eq(bookingsTable.id, req.params.id));

    const [session] = await db.select().from(classSessionsTable).where(eq(classSessionsTable.id, booking.sessionId));
    if (session && session.currentBookings > 0) {
      await db.update(classSessionsTable).set({ currentBookings: session.currentBookings - 1, updatedAt: new Date() }).where(eq(classSessionsTable.id, booking.sessionId));
    }
...
router.patch("/bookings/:id/attendance", requireAuth, async (req, res, next) => {
  try {
    const { status } = z.object({ status: z.enum(["attended", "no_show"]) }).parse(req.body);
    const [updated] = await db
      .update(bookingsTable)
      .set({ status, attendedAt: status === "attended" ? new Date() : null })
